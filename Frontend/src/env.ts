...
export class Config {
  ...
  private convertValue(key: string, value: unknown, type: EnvValueType) {
    switch (type) {
      case 'number': {
        ...
        if (Number.isNaN(num)) {
          throw new Error(`Environment variable ${key} should be a number, got: ${value}`)
        }
        return num
      }
      case 'boolean': {
  ...
  private loadConfig(schema: SchemaDefinition): ConfigType {
    ...
    return entries.reduce((acc, [propName, { type, defaultValue, required }]) => {
      if (process.env[propName] === undefined && required) {
        console.error(`Missing required environment variable: ${propName}`)
        throw new Error()
      }

      const rawValue = this.convertValue(propName, process.env[propName], type) || defaultValue
