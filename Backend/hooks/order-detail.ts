...
export function useCheckOrderDetail<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: number; orderId?: number; productId?: number; quantity?: number }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
